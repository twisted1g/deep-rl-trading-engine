"""Hyperliquid futures client (testnet & mainnet).

Interface used by Trader: Position, fetch_klines, last_price, position,
balance_usdt, quantity_step, round_quantity, market_order, close_position.
"""
...
class HyperliquidFutures:
    ...
    def balance_usdt(self) -> float:
        """Available USDC collateral (perp accountValue or spot USDC fallback)."""
        perp_value = 0.0
        try:
            state = self.info.user_state(self._account_address)
    ...
    def market_order(self, side: str, quantity: float, reduce_only: bool = False) -> dict:
        ...
        is_buy = side == "BUY"
        sz = float(quantity)
        log.info("market_open: coin=%s is_buy=%s sz=%s reduce_only=%s",
